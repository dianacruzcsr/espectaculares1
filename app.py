...
if not st.session_state.df_filtrado.empty:
    ...
    for i, r in df_filtrado.iterrows():
        try:
            ...
            folium.Marker(location=[r["LATITUD"], r["LONGITUD"]], popup=folium.Popup(popup_html, max_width=300), icon=folium.Icon(color=color_marker)).add_to(cluster)
        except Exception as e:
    ...
    with col_dl1:
        ...
        st.download_button(label="⬇️ Descargar Resultados (CSV)", data=csv_file, file_name=f"lugares_cercanos_{datetime.now().strftime('%Y-%m-%d')}.csv", mime="text/csv")
    with col_dl2:
        output = io.BytesIO()
        wb = Workbook()
        ws = wb.active
        ...
        wb.save(output)
        st.download_button(label="⬇️ Descargar Resultados (Excel)", data=output.getvalue(), file_name=f"lugares_cercanos_{datetime.now().strftime('%Y-%m-%d')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    ...
    if st.button("Crear Presentación"):
        if not seleccionados_presentacion:
            st.warning("Por favor, selecciona al menos un espectacular para crear la presentación.")
        else:
            ...
            try:
                ...
                if len(prs.slides) < 2:
                    ...
                    st.download_button(label="⬇️ Descargar Presentación (PPTX)", data=pptx_output.getvalue(), file_name=f"presentacion_espectaculares_{datetime.now().strftime('%Y-%m-%d')}.pptx", mime="application/vnd.openxmlformats-officedocument.presentationml.presentation")
